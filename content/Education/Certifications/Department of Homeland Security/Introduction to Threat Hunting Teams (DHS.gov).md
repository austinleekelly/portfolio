---
created: 2025-04-04T17:24:22-05:00
Institution: Department of Homeland Security
Issue Date: 2018-01-04
modified: 2025-04-29T15:33:09-05:00
NICE Framework Domains:
  - Cyberspace Intelligence
  - Protection and Defense
tags: [Cybersecurity, IT]
---
## 📌 Summary

_The Introduction to Threat Hunting Teams course introduces foundational concepts related to threat hunting in the cyber domain. It outlines the roles and goals of hunting teams, differentiates them from other cyber teams, and explains methods and tools used to interpret cyber threat data._

## 📂 Certification Overview

- **🏆 Certification:** Introduction to Threat Hunting Teams
    
- **🎓 Issuing Organization:** [Department of Homeland Security](https://www.dhs.gov/)
    
- **📆 Date Earned:** 01/04/2018
    
- **📜 Certification Level:** Basic
    

## 🎯 Key Learning Outcomes

- **Threat Hunting Fundamentals** - _Define what threat hunting means, how teams hunt, and how it differs from incident response or pen testing._
    
- **Team Dynamics and Strategy** - _Understand how team goals influence hunting methods and outcomes._
    
- **Threat Analysis Interpretation** - _Recognize and interpret various sources of cyber threat data._
    
- **Threat Modeling** - _Understand three types of threat models and explain one in detail._
    

## 🚀 Skills Acquired

### 🛠 **Core Skills**

- **Technical Skills:** Threat hunting strategy, threat analysis, cyber defense planning
    
- **Soft Skills:** Team-based investigation, analytical thinking, decision making
    

### 💻 **Tools and Technologies**

- **Threat Intelligence Resources** - _Cyber threat landscape interpretation tools and frameworks._
    

### 🗺 **NICE Framework Domains**

- **Cyberspace Intelligence**
    
- **Protection and Defense**
    

## 📜 Certification Details

- **Verification Link:** [[(DHS) Introduction to Threat Hunting Teams - 1.pdf|Introduction to Threat Hunting Teams]]