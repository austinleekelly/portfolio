---
created: 2025-04-15T13:35:18-05:00
modified: 2025-04-29T15:33:08-05:00
Skills_Utilized:
  - Adaptability and Learning
  - Copywriting
  - Customer Journey Mapping
  - Customer Segmentation Strategy
  - Data-Backed Content Research
  - Ecommerce Strategy and Optimization
  - Landing Page Copywriting
  - Lead Generation
  - Persuasion Psychology
  - Problem Solving and Critical Thinking
  - Product Demo Booking Strategy
  - Product Positioning
  - Project Management and Task Prioritization
  - Proofreading and Editing
  - Research and Competitive Analysis
  - SaaS Onboarding and UX Copywriting
  - SEO
  - Strategic Messaging
  - User Polling
  - Web Design
  - Website Optimization and UX Editing
Software:
  - Elementor
  - Figma
  - Hemingway
  - Loom
  - WordPress
  - Yoast
tags: [Case-Study, Copywriting, Ecommerce, Marketing, Marketing-Automation, Owner, Product-Demos, Research, SaaS, SaaS-Marketing, Startup, Testimonial, UX, Web-Design]
YAML Updated: 2025-04-29
---
# 🛍️ Who Spends What  
**Ecommerce Market Analytics SaaS for Shopify Agencies and Developers**  
*Solo project | Market validation, UX design, backend prototyping*

---

## 🔍 Summary
**Who Spends What** was a tool designed to help Shopify agencies and app developers find high-value store prospects based on their spending behavior.

Instead of trying to reach everyone, users could filter stores by their spending on apps, store age, and tools they use. This allowed users to target the right clients who were already investing in their businesses.

---

## 💡 Problem I Tackled
Freelancers, agencies, and app developers often waste time targeting Shopify stores with no clear indication of who’s ready to pay. I aimed to solve this by:

- Tracking when a store starts or stops using a paid app
    
- Showing stores already spending on premium tools
    
- Allowing searches based on spend level, store age, and niche

---

## 🛠 What I Built
- ✅ Wrote all copy and messaging for the landing site
    
- ✅ Designed a **Figma mockup** of the product UI
    
- ✅ Coded a Python backend prototype to track app usage changes
    
- ✅ Built and launched a website (now archived)

---

## 🔥 Validation and Traction
Before launch, I had:

- 💰 **Two agencies** ready to pay **$500/month** for early access
    
- 🤝 One **investor** offering funding for equity
    
- 📬 Positive feedback from cold outreach to target users

---

## 🧭 Why I Didn't Launch
Despite strong feedback, I decided to pause:

- I didn’t want to give up steady freelance income
    
- I didn’t have the bandwidth to support the product long-term
    
- I felt proud to have identified a gap, built a solution, and validated demand

---

## 💬 Customer Quotes from the Landing Page
> “WhoSpendsWhat shows me what stores are using my competitor’s apps. As a Shopify software founder…this fits my needs exactly..”  
> *- [[Long Form Content and Technical Writer for Shoptimized|Bradley Long, Shoptimized Founder]]* 

> “This is an invaluable tool. Instead of casting a big net, this lets me spearfish for prospects - drastically decreasing my time spent prospecting.”  
> *- [[Cold Email Campaign for ZenOKR|Sud Parasar, Founder of ZenOKR]]

---

## 🧠 Skills Demonstrated
- Product research and market positioning  
- UX writing and landing page copy  
- Figma UI prototyping  
- Early backend scripting with Python  
- Customer validation and outbound messaging  
- Founder-style problem solving and initiative  

---

## 📎 Artifacts Available
- ✅ Figma mockup (on request)  
- ✅ Python tracking script (WIP snapshot)  
- 🌐 [Wayback Machine link of original site](https://web.archive.org/web/20211231000000*/whospendswhat.com) (unfortunately not to its full former glory)

---

## ✅ TL;DR
Even though **Who Spends What** didn’t launch, it demonstrates my ability to:

- Spot real-world opportunities
    
- Design useful tools
    
- Communicate a clear offer
    
- Get early market traction
    

I’m proud of this project, not for what it earned, but for what it showed I can do.
